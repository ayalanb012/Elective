    ...
    public  void SubmitClicked(View v)
    {
       ...
       // toast.show();
        ...
        if(validate_form()) {
            ...
            Context ctx = getApplicationContext();
            String res = ServerCalls.Register(ctx,name, mail, pwd, Faculty, Department, Schedule_Json_str); //server call to register the user
            if(!res.equals("invalidMail")) {
                startActivity(new Intent(getApplicationContext(), SignIn.class)); //move user to sign in page
                finish();
            }
            else{
                mail_ET.setText(mail);
                mail_ET.setError("מייל בשימוש");
            }
